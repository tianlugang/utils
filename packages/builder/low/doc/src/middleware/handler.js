...
class Handler {
     ...
     * let {typeText, paramName, paramDesc} = Param.parseParamValue(value);
     *
     * let value = '{number} this is number return value';
     * let {typeText, paramDesc} = Param.parseParamValue(value, true, false, true);
     *
     * let value = '{number}';
     * let {typeText} = Param.parseParamValue(value, true, false, false);
     */
    parseParamValue(value, type = true, name = true, desc = true) {
        value = value.trim();
        ...
        assert(
            typeText || paramName || paramDesc,
            `param is invalid. param = "${value}"`
        );
     ...
     *    let value = '{number} param - this is number param';
     *    let {typeText, paramName, paramDesc} = Param.parseParamValue(value);
     *    let result = Param.parseParam(typeText, paramName, paramDesc);
     */
    parseParam(typeText = null, paramName = null, paramDesc = null) {
        const result = {};
        if (typeText) {
            // check nullable
            if (typeText[0] === "?") {
